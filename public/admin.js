...
async function fetchBoxData() {
  try {
    const response = await fetch('/get-botxes');
...
function generateGrid() {
  ...
  boxData.forEach(box => {
    ...
    gridItem.addEventListener('click', () => handleBoxClick(box)); // Pass the box object to the click handler
...
function handleBoxClick(box) {
  // If box is sold, show the redirect URL
  if (box.isSold && box.imageUrl && box.redirectUrl) {
    window.location.href = box.redirectUrl;
  } else {
    // Show the modal for available boxes with the correct row and column information
    ...
    // Set up the WhatsApp button for purchase
    whatsappBtn.onclick = () => {
